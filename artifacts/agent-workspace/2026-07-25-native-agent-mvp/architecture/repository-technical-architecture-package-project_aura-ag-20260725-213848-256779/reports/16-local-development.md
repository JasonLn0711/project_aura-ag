...
codex --version
codex login
uv run python -c "from aura.agent.reporting import ArchitecturePackageGenerator as G; print(G('.').generate('artifacts/agent-workspace').archive_path)"
find artifacts/agent-workspace -maxdepth 4 -type f | sort
git worktree list
```
